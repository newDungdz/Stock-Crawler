...
if __name__ == "__main__":
    ...
    limit = -1 # Change this value as needed (-1 for no limit)
    processed_count = 0
    ...
    for article_data in links:
        if limit != -1 and processed_count >= limit:
            ...
            break
        logging.info(f"Processing article: {article_data['link']}")
        save_articles_to_json(get_article_info(article_data['link']), "article_data.json")
        processed_count += 1